class PaymentRecordsController < ApplicationController
  ...
  def index
    @payment_records = PaymentRecord.where(:user_id => @current_user.id)
    @payout_records = PayoutRecord.where(:rater_id => @current_user.id)
  end
  ...
  private
    ...
    def payment_record_params
      ...
      params.require(:payment_record).permit(:payment_id, :user_id, :first_name, :last_name, :payment_method, :price, :payment_time, :test_id, :test_name, :payment_status)
    end
end
